...
// Lundi de la semaine contenant d (semaine ISO : lundi=0 … dimanche=6)
function getMon(d) {
  const dt = new Date(d);
  dt.setHours(0, 0, 0, 0);
  const dow = dt.getDay(); // 0=dim,1=lun,...,6=sam
...
// Index du jour dans la semaine (0=lun … 6=dim)
function dayIndex(d) {
...
// ── DEMO DATA ────────────────────────────────────────────────────
function makeDemoRdvs() {
  const rdvs = {};
  const mon = getMon(today);
  const d = i => dk(new Date(mon.getTime() + i * 86400000));
  const add = (ds, sl, name, lines, adjs, status, paid, phone, address, notes, source) => {
    rdvs[`${ds}_${sl}`] = { rdv_key:`${ds}_${sl}`, name, phone, address, notes, source, status, paid, payment:"Virement", lines:JSON.stringify(lines), adjustments:JSON.stringify(adjs) };
  };
  add(tdk,"matin","Samira M.",[{id:"a",type:"standard",category:"canape",itemId:"67p",formula:"premium",qty:1},{id:"b",type:"standard",category:"chaise",itemId:"ch",formula:"basic",qty:6}],[],"confirmed",false,"+32 477 12 34 56","Rue du Trône 14, Ixelles","Canapé tissu beige","Facebook Ads");
  add(tdk,"soir","Karim B.",[{id:"c",type:"standard",category:"matelas",itemId:"2p",formula:"premium",qty:1}],[{id:"x",label:"Frais déplacement",amount:15,isDiscount:false}],"done",true,"+32 496 98 76 54","Av. Louise 200, Uccle","","Recommandation");
  add(d(1),"matin","Nadia K.",[{id:"d",type:"standard",category:"canape",itemId:"45p",formula:"basic",qty:1},{id:"e",type:"standard",category:"tapis",itemId:"4m",formula:"basic",qty:1}],[],"confirmed",false,"+32 488 55 44 33","Chée de Waterloo 85","","Instagram");
  add(d(2),"midi","Omar F.",[{id:"f",type:"standard",category:"voiture",itemId:"5s",formula:"premium",qty:1}],[{id:"y",label:"Supplément urgence",amount:20,isDiscount:false}],"confirmed",false,"+32 479 33 22 11","Parking Woluwe","BMW X5","WhatsApp");
  add(d(3),"matin","Fatima Z.",[{id:"g",type:"standard",category:"canape",itemId:"89p",formula:"premium",qty:1},{id:"h",type:"standard",category:"matelas",itemId:"1p",formula:"basic",qty:1}],[],"confirmed",false,"+32 472 99 88 77","Bd Whitlock 45","","Google");
  add(d(4),"soir","Youssef A.",[{id:"i",type:"standard",category:"chaise",itemId:"ch",formula:"basic",qty:8},{id:"j",type:"custom",label:"Remise fidélité",qty:1,unitPrice:-15}],[],"done",true,"+32 485 11 22 33","Rue Goffart 12","","Recommandation");
  add(d(6),"matin","Leila D.",[{id:"p",type:"standard",category:"canape",itemId:"23p",formula:"premium",qty:1}],[],"confirmed",false,"+32 471 00 11 22","Rue Belliard 10","Dimanche matin","Recommandation");
  const lmon = new Date(mon.getTime() - 7 * 86400000);
  const ld = i => dk(new Date(lmon.getTime() + i * 86400000));
  add(ld(0),"matin","Sophie D.",[{id:"k",type:"standard",category:"canape",itemId:"23p",formula:"premium",qty:1}],[],"done",true,"+32 471 44 55 66","Rue de la Loi 12","","Facebook Ads");
  add(ld(1),"midi","Michel T.",[{id:"l",type:"standard",category:"matelas",itemId:"2p",formula:"basic",qty:1},{id:"m",type:"standard",category:"tapis",itemId:"6m",formula:"basic",qty:1}],[],"done",true,"+32 494 77 88 99","Av. des Arts 5","","Google");
  add(ld(3),"soir","Hassan B.",[{id:"o",type:"standard",category:"voiture",itemId:"5s",formula:"basic",qty:1}],[],"done",true,"+32 482 66 77 88","Av. Toison d'Or 19","","Instagram");
  add(ld(6),"matin","Amina R.",[{id:"q",type:"standard",category:"matelas",itemId:"1p",formula:"premium",qty:1}],[],"done",true,"+32 476 33 44 55","Rue du Midi 22","Dimanche","Recommandation");
  return rdvs;
}
...
// ── RDV FORM ─────────────────────────────────────────────────────
function RDVForm({ initial, onSave }) {
  ...
  return (
    <div>
      ...
      <button disabled={!ok} onClick={()=>onSave(form)} style={{width:"100%",background:ok?"#0f172a":"#e2e8f0",color:ok?"white":"#94a3b8",border:"none",borderRadius:12,padding:"13px",fontWeight:800,fontSize:14,cursor:ok?"pointer":"default"}}>✅ Confirmer le RDV</button>
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        ...
        <button onClick={onDelete} style={{background:"#f1f5f9",color:"#64748b",border:"none",borderRadius:10,padding:"11px",fontWeight:700,fontSize:12,cursor:"pointer"}}>🗑️ Supprimer</button>
      </div>
    </div>
  );
}
...
export default function App() {
  const [rdvs, setRdvs]           = useState(() => makeDemoRdvs());
  ...
  // 7 dates de la semaine affichée
  ...
  const saveRdv = form => {
    ...
    setRdvs(r => ({...r, [k]: { rdv_key:k, ...form, lines:JSON.stringify(form.lines||[]), adjustments:JSON.stringify(form.adjustments||[]), total_amount:calcTotals(form.lines||[],form.adjustments||[]).total }}));
    setFormData(null); setDetailData(null);
  };
  const openDetail = (key, slot, date) => { const rdv=rdvs[key]; if(!rdv)return; setDetailData({rdv,slot,key,date}); };
  const openForm   = (key, slot, date, existing=null) => {
    setFormData({ key, slot, date, initial: existing ? {...existing, lines:JSON.parse(existing.lines||"[]"), adjustments:JSON.parse(existing.adjustments||"[]")} : null });
  };
  ...
  const navPrev = () => {
    if (calView==="month") setMonthDate(d => { const n=new Date(d); n.setMonth(d.getMonth()-1); return n; });
    else setMonday(m => { const d=new Date(m); d.setDate(d.getDate()-7); return d; });
  };
  const navNext = () => {
    if (calView==="month") setMonthDate(d => { const n=new Date(d); n.setMonth(d.getMonth()+1); return n; });
    else setMonday(m => { const d=new Date(m); d.setDate(d.getDate()+7); return d; });
  ...
  const navToday = () => {
    const m = getMon(today);
    setMonday(m);
    setSelDay(dayIndex(today));
    setMonthDate(new Date(today.getFullYear(), today.getMonth(), 1));
  };

  const navLabel = () => {
    if (calView==="month") return `${MONTHS_FR[monthDate.getMonth()]} ${monthDate.getFullYear()}`;
    const e = dates[6];
    return `${fmtShort(monday)} – ${fmtShort(e)}`;
  };
  ...
  const WeekView = () => (
    <div style={{padding:"14px 10px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
        {[
          {ico:"💰",lbl:"CA semaine",val:fmt(wCA),bg:"#f0fdf4",c:"#16a34a",bdr:"#bbf7d0"},
          {ico:"✅",lbl:"Encaissé",val:fmt(wPaid),bg:"#d1fae5",c:"#059669",bdr:"#6ee7b7"},
          {ico:"⏳",lbl:"À encaisser",val:fmt(wCA-wPaid),bg:"#fef9c3",c:"#d97706",bdr:"#fde68a"},
          {ico:"📋",lbl:`RDV (${wDone}/${wActive.length} faits)`,val:wActive.length,bg:"#eff6ff",c:"#2563eb",bdr:"#bfdbfe"},
        ].map(({ico,lbl,val,bg,c,bdr})=>(
          <div key={lbl} style={{background:bg,borderRadius:16,padding:"12px 14px",border:`1.5px solid ${bdr}`}}>
            <div style={{fontSize:11,fontWeight:700,color:c,marginBottom:5}}>{ico} {lbl}</div>
          ...
          return(
            <div key={di} style={{background:isT?"#3b82f6":isSun?"#fdf4ff":"white",borderRadius:16,border:`2px solid ${isT?"#3b82f6":isSun?"#e9d5ff":"#e8edf2"}`,overflow:"hidden",boxShadow:isT?"0 4px 20px rgba(59,130,246,0.28)":"0 1px 4px rgba(0,0,0,0.05)"}}>
              ...
              <div style={{textAlign:"center",padding:"8px 2px 6px",borderBottom:`1px solid ${isT?"rgba(255,255,255,0.2)":isSun?"#e9d5ff":"#f1f5f9"}`}}>
                <div style={{fontSize:8.5,fontWeight:700,color:isT?"rgba(255,255,255,0.8)":isSun?"#9333ea":"#94a3b8",textTransform:"uppercase",letterSpacing:"0.3px"}}>{DAYS_SHORT[di]}</div>
                <div style={{fontSize:18,fontWeight:800,color:isT?"white":isSun?"#9333ea":"#0f172a",lineHeight:1.1}}>{date.getDate()}</div>
                <div style={{fontSize:8.5,fontWeight:500,color:isT?"rgba(255,255,255,0.65)":isSun?"#a855f7":"#94a3b8"}}>{MONTHS_S[date.getMonth()]}</div>
              </div>
              {/* Créneaux */}
              <div style={{padding:"5px 4px",background:isT?"transparent":isSun?"#fdf4ff":"#f8fafc"}}>
                {SLOTS.map(slot=>{
                  ...
                  if(r){
                    ...
                    return(
                      <div key={slot.id} onClick={()=>openDetail(k,slot,date)} style={{background:"white",borderRadius:10,padding:"6px 5px",marginBottom:5,cursor:"pointer",border:`1.5px solid ${slot.border}`,transition:"transform 0.1s"}} onMouseEnter={e=>e.currentTarget.style.transform="scale(1.04)"} onMouseLeave={e=>e.currentTarget.style.transform=""}>
                        <div style={{display:"flex",alignItems:"center",gap:2,marginBottom:2}}>
                          <div style={{width:5,height:5,borderRadius:"50%",background:slot.dot,flexShrink:0}}/>
                          <div style={{fontSize:7.5,fontWeight:700,color:slot.color,lineHeight:1.2}}>{slot.time}<br/>–{slot.end}</div>
                        </div>
                        <div style={{fontSize:9,fontWeight:800,color:"#0f172a",lineHeight:1.2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginBottom:1}}>{r.name}</div>
                        ...
                        <div style={{marginTop:2,display:"flex",gap:2,flexWrap:"wrap"}}>
                          <span style={{background:sc.bg,color:sc.color,fontSize:7,fontWeight:700,padding:"1px 3px",borderRadius:3}}>{sc.label}</span>
                          {r.paid&&<span style={{background:"#d1fae5",color:"#059669",fontSize:7,fontWeight:700,padding:"1px 3px",borderRadius:3}}>💳</span>}
                        </div>
                      </div>
                    );
                  }
                  return(
                    <div key={slot.id} onClick={()=>openForm(k,slot,date)} style={{background:isT?"rgba(255,255,255,0.15)":"white",borderRadius:10,padding:"5px 4px",marginBottom:5,cursor:"pointer",border:`1.5px dashed ${isT?"rgba(255,255,255,0.35)":slot.border}`,minHeight:50,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,transition:"all 0.1s"}} onMouseEnter={e=>e.currentTarget.style.background=isT?"rgba(255,255,255,0.25)":"#f0f9ff"} onMouseLeave={e=>e.currentTarget.style.background=isT?"rgba(255,255,255,0.15)":"white"}>
                      <div style={{display:"flex",alignItems:"center",gap:2}}>
                        <div style={{width:4,height:4,borderRadius:"50%",background:isT?"rgba(255,255,255,0.5)":slot.dot,opacity:0.5}}/>
                        <div style={{fontSize:7.5,fontWeight:700,color:isT?"rgba(255,255,255,0.65)":slot.color,lineHeight:1.2}}>{slot.time}<br/>–{slot.end}</div>
                      </div>
      ...
      <div>
        ...
        <div style={{background:"white",borderBottom:"1px solid #f1f5f9",display:"flex",overflowX:"auto",WebkitOverflowScrolling:"touch",padding:"0 8px"}}>
          {dates.map((date,di)=>{
            const isSun=di===6;
            return(
              <button key={di} onClick={()=>setSelDay(di)} style={{flexShrink:0,background:"none",border:"none",cursor:"pointer",padding:"7px 10px",borderBottom:`3px solid ${selDay===di?"#3b82f6":"transparent"}`,transition:"all 0.15s"}}>
        ...
        {dayCA>0&&(
          <div style={{padding:"8px 16px",background:"white",borderBottom:"1px solid #f1f5f9"}}>
            <div style={{background:"#f0fdf4",borderRadius:10,padding:"8px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:12,color:"#16a34a",fontWeight:700}}>💰 CA du jour</span>
              <span style={{fontSize:18,fontWeight:800,color:"#16a34a"}}>{fmt(dayCA)}</span>
            </div>
          </div>
        )}
        <div style={{padding:"14px 16px"}}>
          ...
          {SLOTS.map(slot=>{
            ...
            if(r){
              ...
              const {total}=calcTotals(lines,adjs), sc=STATUS_CFG[r.status]||STATUS_CFG.confirmed;
              return(
                <div key={slot.id} onClick={()=>openDetail(k,slot,dates[selDay])} style={{borderRadius:14,padding:"12px 14px",cursor:"pointer",background:slot.light,border:`1.5px solid ${slot.border}`,marginBottom:8,transition:"all 0.12s"}} onMouseEnter={e=>{e.currentTarget.style.transform="scale(1.01)";e.currentTarget.style.boxShadow=`0 4px 16px ${slot.color}20`;}} onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";}}>
                  <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:5}}>
                    <div style={{width:7,height:7,borderRadius:"50%",background:slot.dot}}/>
                    <span style={{fontSize:11,fontWeight:700,color:slot.color,textTransform:"uppercase"}}>{slot.time} – {slot.end}</span>
                  </div>
            ...
            return(
              <div key={slot.id} onClick={()=>openForm(k,slot,dates[selDay])} style={{borderRadius:14,padding:"12px",cursor:"pointer",border:"1.5px dashed #e2e8f0",minHeight:72,display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",gap:4,marginBottom:8,transition:"all 0.15s"}} onMouseEnter={e=>{e.currentTarget.style.background="#f8fafc";e.currentTarget.style.borderColor=slot.dot+"70";}} onMouseLeave={e=>{e.currentTarget.style.background="";e.currentTarget.style.borderColor="#e2e8f0";}}>
  ...
  const MonthView = () => {
    ...
    const getRd=day=>{const ds=`${year}-${String(month+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;return SLOTS.map(slot=>{const r=getR(ds,slot.id);return r?{...r,slot,key:`${ds}_${slot.id}`}:null;}).filter(Boolean);};
    const cells=[];
        ...
        <div style={{padding:"6px 10px 20px"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
            {cells.map((day,i)=>{
              ...
              const isSun=i%7===6;
              return(
                <div key={i} onClick={()=>{setMonday(getMon(new Date(ds)));setSelDay(dayIndex(new Date(ds)));setCalView("day");}} style={{minHeight:70,background:isT?"#eff6ff":isSun?"#fdf4ff":"white",borderRadius:10,padding:"4px",cursor:"pointer",border:isT?"1.5px solid #bfdbfe":isSun?"1.5px solid #e9d5ff":"1.5px solid #f1f5f9",transition:"all 0.12s"}} onMouseEnter={e=>e.currentTarget.style.background=isT?"#dbeafe":isSun?"#f3e8ff":"#f8fafc"} onMouseLeave={e=>e.currentTarget.style.background=isT?"#eff6ff":isSun?"#fdf4ff":"white"}>
                  <div style={{fontWeight:isT?800:600,fontSize:11,color:isT?"#3b82f6":isSun?"#9333ea":"#0f172a",lineHeight:1,marginBottom:2,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  {dRdvs.length>2&&<div style={{fontSize:8,color:"#64748b",fontWeight:600}}>+{dRdvs.length-2}</div>}
                </div>
              );
            })}
          </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#eef2f7",fontFamily:"'Segoe UI',system-ui,-apple-system,sans-serif",maxWidth:720,margin:"0 auto"}}>
      ...
      <div style={{background:"#0f172a",padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 16px rgba(0,0,0,0.35)"}}>
        <div>
          <div style={{color:"white",fontWeight:800,fontSize:17,letterSpacing:"-0.3px"}}>StayClean</div>
          <div style={{color:"#64748b",fontSize:10,fontWeight:500}}>Gestion interventions</div>
        </div>
        ...
        <div style={{background:"white",borderBottom:"1px solid #e8edf2",padding:"10px 16px"}}>
          <div style={{display:"flex",gap:6,marginBottom:10}}>
            {[{id:"day",label:"Jour"},{id:"week",label:"Semaine"},{id:"month",label:"Mois"}].map(({id,label})=>(
              <button key={id} onClick={()=>setCalView(id)} style={{flex:1,background:calView===id?"#0f172a":"#f1f5f9",color:calView===id?"white":"#64748b",border:"none",borderRadius:10,padding:"9px 0",fontWeight:700,fontSize:12,cursor:"pointer",transition:"all 0.15s"}}>{label}</button>
            ))}
          </div>
      ...
      <Sheet open={!!detailData} onClose={()=>setDetailData(null)} title="Détail intervention">
        {detailData&&<RDVDetail rdv={detailData.rdv} slot={detailData.slot}
          ...
          onStatusChange={s=>{setRdvs(r=>({...r,[detailData.key]:{...r[detailData.key],status:s}}));setDetailData(d=>({...d,rdv:{...d.rdv,status:s}}));}}
          onTogglePaid={()=>{setRdvs(r=>({...r,[detailData.key]:{...r[detailData.key],paid:!r[detailData.key].paid}}));setDetailData(d=>({...d,rdv:{...d.rdv,paid:!d.rdv.paid}}));}}
          onDelete={()=>{setRdvs(r=>{const n={...r};delete n[detailData.key];return n;});setDetailData(null);}}
        />}
      </Sheet>

      <Sheet open={!!formData} onClose={()=>setFormData(null)} title={formData?.initial?"Modifier le RDV":"Nouveau RDV"}>
        {formData&&<RDVForm initial={formData.initial} onSave={saveRdv}/>}
      </Sheet>
    </div>
  );
}
